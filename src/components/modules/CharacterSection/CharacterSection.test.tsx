...
jest.mock('../CharacterCard/CharacterCard', () => ({
  CharacterCard: ({ character, onClick, isActive }: any) => (
    <div
...
// Mock de componentes de PrimeReact
...
jest.mock('primereact/paginator', () => ({
  Paginator: ({ onPageChange, first, rows, totalRecords, template }: any) => (
    <div data-testid="paginator">
      <button
        data-testid="next-page"
        onClick={() => onPageChange({ page: 1, rows: 10 })}
      >
        Next
      </button>
...
jest.mock('primereact/inputtext', () => ({
  InputText: ({ placeholder, onChange, className }: any) => (
    <input
...
jest.mock('primereact/dropdown', () => ({
  Dropdown: ({ options, onChange, value, className }: any) => (
    <select
